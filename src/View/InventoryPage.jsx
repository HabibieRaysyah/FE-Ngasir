...
export default function InventoryPage() {
  ...
  const getCurrentStock = async (productId) => {
    try {
      const findProduct = products.filter((fil) => fil.status == true).find((item) => item.id == productId)
  ...
  const getStoreUser = async () => {
    try {
      ...
      const filteredUser = res.data.data.filter((store) => store.store_id == id).map((stor) => stor.User)
  ...
  const handlePost = async () => {
    try {
      ...
      const res = await api.post(`/inventory/`, dataToSend);
  ...
  const handleUpdate = async () => {
    try {
      ...
      const res = await api.put(`/inventory/`, dataToSend);
  ...
  const handleDelete = async () => {
    try {
      const res = await api.delete(`/inventory/${deleteData.id}`);
